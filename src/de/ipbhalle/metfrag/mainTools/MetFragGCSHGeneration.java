...
public class MetFragGCSHGeneration {
...
//		String pathToPreCalculatedCML = "/home/swolf/MOPAC/BondOrderTests/Hill_ProofOfConcept/mopac_2400/";
		...
		Map<String, File> candidateFolderMap = new HashMap<String, File>(); 
		File[] candidateFolder = new File(pathToPreCalculatedCML).listFiles();
		Arrays.sort(candidateFolder);
		for (File file : candidateFolder) {
			candidateFolderMap.put(file.getName().split("_")[0], file);
		}
		...
		for (File file : spectraFolder) {
			...
			String number = file.getName().split("_")[1].split("\\.")[0].substring(2);
...
//			"/home/swolf/MOPAC/100spec_SDF_GC-EIMS/spectra/RandSel100Third_00013.mf" "2011-11-29_13-33-33" "/home/swolf/MOPAC/100spec_SDF_GC-EIMS/sdf_calculated/013_38120_C1H5Si1Br1/"
			out.write("java -Dproperty.file.path=" + configPath + " -Xms1500m -Xmx5500m -jar " + pathToJar + " \"" + file.getPath() + "\" \"" + dateString + "\" \"" + candidateFolderMap.get(number) + "/\"");
		  	out.close();
